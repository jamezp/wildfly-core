...
final class HandlerConfigurationImpl extends AbstractPropertyConfiguration<Handler, HandlerConfigurationImpl> implements HandlerConfiguration {
    ...
    HandlerConfigurationImpl(final LogContextConfigurationImpl configuration, final String name, final String moduleName, final String className, final String[] constructorProperties) {
        super(Handler.class, configuration, configuration.contextConfiguration::removeHandler, configuration.getHandlerConfigurations(), name, moduleName, className, constructorProperties);
    }
    ...
    private void setFormatterName(final ValueExpression<String> expression) {
        ...
        configuration.addAction(new ConfigAction<Void>() {
            ...
            public void applyPostCreate(final Void param) {
                configuration.contextConfiguration.getHandler(getName()).setFormatter(formatterName == null ? null : configuration.contextConfiguration.getFormatter(formatterName));
            }
    ...
    public Handler getInstance() {
        return contextConfiguration.getHandler(getName());
    }
    ...
    private void setLevelValueExpression(final ValueExpression<String> expression) {
        ...
        configuration.addAction(new ConfigAction<Level>() {
            ...
            public void applyPostCreate(final Level param) {
                contextConfiguration.getHandler(getName()).setLevel(param);
            }
    ...
    private void setFilter(final ValueExpression<String> expression) {
        ...
        configuration.addAction(new ConfigAction<ObjectProducer>() {
            ...
            public void applyPostCreate(final ObjectProducer param) {
                contextConfiguration.getHandler(getName()).setFilter((Filter) param.getObject());
            }
    ...
    private void setEncoding(final ValueExpression<String> expression) {
        ...
        configuration.addAction(new ConfigAction<Void>() {
            ...
            public void applyPostCreate(final Void param) {
                try {
                    contextConfiguration.getHandler(getName()).setEncoding(encoding);
    ...
    private void setErrorManagerName(final ValueExpression<String> expression) {
        ...
        configuration.addAction(new ConfigAction<Void>() {
            ...
            public void applyPostCreate(final Void param) {
                contextConfiguration.getHandler(getName()).setErrorManager(errorManagerName == null ? null : contextConfiguration.getErrorManager(errorManagerName));
            }
    ...
    public void setHandlerNames(final String... names) {
        ...
        configuration.addAction(new ConfigAction<Void>() {
            ...
            public void applyPostCreate(final Void param) {
                final ExtHandler handler = (ExtHandler) contextConfiguration.getHandler(getName());
                final int length = stringsArray.length;
                final Handler[] handlers = new Handler[length];
                for (int i = 0; i < length; i ++) {
                    handlers[i] = contextConfiguration.getHandler(stringsArray[i]);
                }
    ...
    public boolean addHandlerName(final String name) {
        ...
        configuration.addAction(new ConfigAction<Void>() {
            ...
            public void applyPostCreate(final Void param) {
                final ExtHandler handler = (ExtHandler) contextConfiguration.getHandler(getName());
                handler.addHandler(contextConfiguration.getHandler(name));
            }
    ...
    public boolean removeHandlerName(final String name) {
        ...
        configuration.addAction(new ConfigAction<Handler>() {
            public Handler validate() throws IllegalArgumentException {
                return contextConfiguration.getHandler(name);
            }
    ...
    @Override
    ConfigAction<Handler> getConstructAction(final BiConsumer<String, Handler> preCreate) {
        return new ConstructAction(preCreate) {
            @Override
            public void rollback() {
                final Supplier<Handler> handler = contextConfiguration.removeHandler(getName());
                if (handler != null) {
                    try {
                        handler.get().close();
